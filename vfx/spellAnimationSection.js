...
export function renderSpellAnimationSection({ animations = {}, library, spellId = "", editable = true } = {}) {
  ...
  }).join("")}</div><button type="button" data-cc-action="preview-spell-animations" data-spell-id="${escape(spellId)}">Preview Full Spell</button><button type="button" data-cc-action="edit-spell-animations" data-spell-id="${escape(spellId)}">Choose, create or upload animations</button><p class="small">Saved with this spell when you save the character draft. No global assignment save.</p></section>`;
}
