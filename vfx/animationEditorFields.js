import { ANIMATION_TAGS, applyBehaviorPlacementDefaults } from "./animationLibrary.js";
import { ANIMATION_PRESETS } from "./animationPlayback.js";
import { ANIMATION_FAMILIES, getAnimationStyles } from "./animationFamilies.js";
...
export const EDITOR_FIELDS = {
  family: ["family", "magic", "text"], style: ["style", "custom", "text"],
  ...
  spawn: ["placement.spawnAt", "map", "text"], "override-placement": ["placement.override", true, "check"], "follow-source": ["placement.followSource", false, "check"], "follow-target": ["placement.followTarget", false, "check"], persist: ["placement.persist", false, "check"], duration: ["placement.duration", 0], behavior: ["behavior", "static", "text"],
...
export function animationFormMarkup() {
  return `<div class="hg-animation-edit-heading"><h3 data-animation-editor-title>Create animation</h3>${toggle("advanced-mode", "Advanced Settings")}</div>
    <div class="hg-animation-family-config">${select("family", "Family", ANIMATION_FAMILIES.map(family => [family, family[0].toUpperCase() + family.slice(1)]))}${select("style", "Style", getAnimationStyles("magic").map(style => [style.value, style.label]))}<button type="button" data-animation-apply-style-defaults>Apply style defaults</button></div><input data-animation-editor-type type="hidden"><details data-advanced open><summary>Universal behavior presets</summary><div class="hg-animation-preset-bar">${select("preset", "Start with a preset", [["", "Choose a starting point"], ...Object.entries(ANIMATION_PRESETS).map(([key, p]) => [key, p.name])])}<button type="button" data-animation-apply-preset>Apply preset</button></div></details>
    ${section("basic", "Basic", group(`<label>Name<input data-animation-name maxlength="120" required placeholder="My new effect"></label><label>Collection<input data-animation-collections placeholder="Gary, Sword Pack" maxlength="1200"><small>User folders; separate multiple collections with commas.</small></label>`) +
    ...
    ${section("sprite", "Sprite Sheet", `<div data-animation-upload-zone><label class="hg-animation-upload">Drop a sprite sheet here or choose a file<input data-animation-file type="file" accept="image/png,image/webp,image/jpeg"><small>PNG, WebP or JPEG · up to 8 MB</small></label><img data-animation-upload-thumbnail alt="Uploaded sprite sheet" hidden style="max-width:120px;max-height:100px"><button type="button" data-animation-replace-sprite>Replace Sprite</button></div><p data-animation-file-info class="hg-animation-hint"></p>
      ...
      <div class="hg-animation-grid-suggestion-panel"><strong>Grid suggestions</strong><small>Dimension-based suggestions only; your choice is never changed automatically.</small><div data-animation-grid-suggestions class="hg-animation-grid-suggestions"></div></div>
      <div data-animation-sheet-editor class="hg-animation-sheet-editor" hidden>
        <div class="hg-animation-sheet-editor-heading"><strong>Visual frame grid</strong><output data-animation-sheet-grid-label></output></div>
        <label class="hg-animation-toggle hg-animation-frame-box-toggle"><input data-animation-edit-frame-boxes type="checkbox">Edit Frame Boxes</label>
        <div data-animation-frame-alignment-controls class="hg-animation-frame-alignment-controls" hidden>
          ${group(input("grid-offset-x", "Grid offset X (px)", -16384, 16384) + input("grid-offset-y", "Grid offset Y (px)", -16384, 16384) + input("frame-inset-x", "Frame inset X (px)", 0, 64) + input("frame-inset-y", "Frame inset Y (px)", 0, 64))}
          <div class="hg-animation-frame-nudges" role="group" aria-label="Nudge frame grid"><button type="button" data-animation-frame-nudge="left">← Left 1px</button><button type="button" data-animation-frame-nudge="right">Right 1px →</button><button type="button" data-animation-frame-nudge="up">↑ Up 1px</button><button type="button" data-animation-frame-nudge="down">Down 1px ↓</button><button type="button" data-animation-reset-alignment>Reset Alignment</button></div>
          <p class="hg-animation-hint">Drag the boundary overlay to reposition the grid. Hold Shift while nudging for 5px. Only sampling metadata changes; the uploaded sheet stays untouched.</p>
        </div>
        <div data-animation-sheet-grid-wrap class="hg-animation-sheet-grid-wrap"><img data-animation-sheet-grid-image alt="Sprite sheet with numbered cell overlay"><div data-animation-frame-grid class="hg-animation-frame-grid" role="grid" aria-label="Sprite frames"></div><div data-animation-frame-alignment-overlay class="hg-animation-frame-alignment-overlay" aria-label="Draggable frame box grid" hidden></div><div data-animation-frame-intended class="hg-animation-frame-diagnostic hg-animation-frame-intended" hidden></div><div data-animation-frame-sampled class="hg-animation-frame-diagnostic hg-animation-frame-sampled" hidden></div><div data-animation-neighbor-bleed-bands class="hg-animation-neighbor-bleed-bands" hidden></div></div>
        <div class="hg-animation-frame-scrubber"><button type="button" data-animation-frame-first aria-label="First frame">|&lt;</button><button type="button" data-animation-frame-previous aria-label="Previous frame">&lt;</button><output data-animation-frame-position>FRAME 1 / 1</output><button type="button" data-animation-frame-next aria-label="Next frame">&gt;</button><button type="button" data-animation-frame-last aria-label="Last frame">&gt;|</button></div>
        <input data-animation-frame-scrubber type="range" min="0" max="0" value="0" aria-label="Preview frame">
        <div class="hg-animation-frame-preview"><div data-animation-frame-crop-art class="hg-animation-frame-crop-art"><span data-animation-frame-content-bounds></span></div><div><p data-animation-frame-selection-summary class="hg-animation-hint"></p><label class="hg-animation-toggle"><input data-animation-frame-crop-toggle type="checkbox">Show crop / padding analysis</label><label class="hg-animation-toggle"><input data-animation-neighbor-bleed type="checkbox">Show Neighbor Bleed</label><output data-animation-frame-padding class="hg-animation-hint" hidden></output><output data-animation-neighbor-bleed-summary class="hg-animation-hint" hidden></output><button type="button" data-animation-trim-empty>Trim Empty Start / End Frames</button></div></div>
        <p class="hg-animation-hint">Use each checkbox to include or skip a cell. Click its frame number or use the scrubber to inspect it. This changes playback metadata only.</p>
      </div>
    ...
    ${section("layers", "Layers", `<p class="hg-animation-hint">Combine saved animations without copying their sprite data. Layers play with this animation in preview and combat.</p><div data-animation-layers-editor><div class="hg-animation-layer-toolbar"><output data-animation-layer-count>0 / 8 layers</output><button type="button" data-animation-add-layer>Add Layer</button></div><p data-animation-layer-empty class="hg-animation-hint">No visual layers yet.</p><div data-animation-layer-list></div></div>`, false, true)}
    ${section("timeline", "Timeline", `<p class="hg-animation-hint">Drag layer blocks to set their start delay. Event diamonds and sound triangles use the shared player frame timing.</p><div data-animation-timeline-editor></div>`, false, true)}
    ${section("size", "Size & Position", input("scale", "Uniform scale ×", .1, 8, .1) + group(select("behavior", "Animation behavior", [["static", "Static"], ["source-effect", "Source Effect"], ["target-effect", "Target Effect"], ["melee", "Melee"], ["projectile", "Projectile"], ["beam", "Beam"], ["aura", "Aura"], ["ground", "Ground Effect"], ["screen", "Screen Effect"], ["summon", "Summon"], ["attached", "Attached Effect"]])) + toggle("override-placement", "Override Placement") + `<p data-animation-placement-summary class="hg-animation-hint"></p><div data-advanced>${toggle("lock", "Lock proportions")}
      ...
      <span class="hg-animation-field-label">Anchor / pivot</span><div class="hg-animation-pivots" role="group" aria-label="Anchor / pivot">${["Top Left", "Top", "Top Right", "Left", "Center", "Right", "Bottom Left", "Bottom", "Bottom Right"].map((name, i) => `<button type="button" data-anchor="${i % 3 / 2},${Math.floor(i / 3) / 2}" title="${name}" aria-label="${name}">◉</button>`).join("")}</div></div>
      <div data-animation-placement-controls>${group(select("spawn", "Spawn position", [["source", "Caster / Source"], ["target", "Target"], ["between", "Midpoint"], ["map", "World / Map position"], ["source-to-target", "Source to target"], ["source-toward-target", "Source toward target"]]) + select("facing", "Facing", [["fixed", "Fixed"], ["face-target", "Face Target"]]))}
      ${group(input("toward-offset", "Offset toward target (px)", 0, 1000) + input("visual-reach", "Visual reach (ft, optional)", 0, 1000))}${toggle("fixed-map", "Fixed to map")}${toggle("follow-source", "Follow source")}${toggle("follow-target", "Follow target")}</div>
      <div data-advanced>${group(select("area-shape", "Visual area", ["point", "circle", "cone", "line", "rectangle", "self"]) + select("area-unit", "Area units", ["ft", "px"]) + input("area-radius", "Radius (0 = manual size)", 0, 1000) + input("area-width", "Area width", 0, 1000) + input("area-length", "Area length", 0, 1000))}<p class="hg-animation-hint">Feet use the supplied map grid. Scale remains an additional multiplier.</p>
      ${toggle("persist", "Persist after animation")}${input("duration", "Duration (seconds, 0 = automatic / until Stop)", 0, 60, .1)}
    ...
    ${section("direction", "Direction", group(input("rotation", "Rotation (degrees)", -3600, 3600) + select("source-direction", "Source direction in the artwork", [["up", "Up ↑"], ["right", "Right →"], ["down", "Down ↓"], ["left", "Left ←"]])) + toggle("flip-x", "Flip Horizontal") + toggle("flip-y", "Flip Vertical"), true)}
...
export function readAnimationFields(field) {
  ...
  return result.placement.override ? result : applyBehaviorPlacementDefaults(result);
}
