...
export function animationFormMarkup() {
  return `<div class="hg-animation-edit-heading"><h3 data-animation-editor-title>Create animation</h3>${toggle("advanced-mode", "Advanced Settings")}</div>
    ...
    ${section("sprite", "Sprite Sheet", `<label class="hg-animation-upload">Sprite sheet<input data-animation-file type="file" accept="image/png,image/webp,image/jpeg"><small>PNG, WebP or JPEG · up to 8 MB</small></label><p data-animation-file-info class="hg-animation-hint"></p>
    ...
    ${section("size", "Size & Position", input("scale", "Uniform scale ×", .1, 8, .1) + `<div data-advanced>${toggle("lock", "Lock proportions")}
      ...
      ${group(select("spawn", "Spawn position", [["source", "Caster / Source"], ["target", "Target"], ["between", "Between source and target"], ["map", "Map position"]]) + select("behavior", "Animation behavior", [["static", "Static / Attached"], ["projectile", "Projectile"], ["beam", "Stretch between source and target"]]))}
