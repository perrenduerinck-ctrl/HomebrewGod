...
export function getAnimationDeletionPolicy(animation, knownReferences = []) {
  ...
  return {
    ...
    message: persistent
      ? `${UNKNOWN_REMOTE_ANIMATION_WARNING} ${known} Remove from My Library deletes the saved metadata and removes known loaded references only after sync succeeds. The hosted sprite is retained. Other saved references may need replacement or use safe fallback. Save affected characters before removing metadata. Replace Known References keeps this animation in your library.`
