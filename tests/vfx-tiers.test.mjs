...
import { TIER_EFFECT_DEFINITIONS, LESSER_SPRITE_ASSETS, TIER_SPRITE_ASSETS,
...
test("base-level grid policy is 4x4 / 5x5 / 6x6 and missing art never downgrades silently", () => {
  ...
  assert.equal(getTierSpriteAsset("fire", 7), null, "await real 36-frame sheets");
...
test("all supplied atlases exist; frame windows and dimensions match each actual layout", () => {
  ...
  for (const [theme, path] of [...Object.entries(LESSER_SPRITE_ASSETS), ...Object.entries(TIER_SPRITE_ASSETS)]) {
  ...
  for (const effect of TIER_EFFECT_DEFINITIONS) {
    const sprite = effect.sprite, grid = effect.id.startsWith("lesser-") ? 4 : 5;
    assert.equal(sprite.columns, grid); assert.equal(sprite.rows, grid);
    assert.equal(sprite.frameCount, grid ** 2);
...
test("4x4, 5x5 and future 6x6 frame playback reaches its last cell and cancels all RAF work", () => {
  for (const level of [0, 3, 7]) {
    const grid = getSpellSpriteTier(level), callbacks = new Map(); let next = 0, removed = 0;
...
test("every mapped spell keeps its real rules and has a working DM-only preview entry", () => {
  ...
  for (const id of ids) {
    ...
    assert.ok(spell.level >= 1 && spell.level <= 6, id);
