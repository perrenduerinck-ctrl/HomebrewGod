...
// =====================================================
  ...
  async function createMonsterLinkedToken(
    monster
  ) {
    const roomCode =
      deps.getCurrentRoomCode
    ...
    if (!roomCode) {
      throw new Error(
        "Open a room before creating a monster token."
      );
    }

    if (!isDM) {
      throw new Error(
        "Only the DM can create monster tokens."
      );
    }
    ...
    if (!target.mapMode) {
      throw new Error(
        "Load a battle map or puzzle map before creating the monster token."
      );
    }
